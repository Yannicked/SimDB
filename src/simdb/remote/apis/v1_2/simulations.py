...
def _get_json_aware(force: bool = False, silent: bool = False):
    ...
    if "gzip" in enc:
        # Keep raw bytes if not actually gzipped
    ...
    try:
        loads = flask_current_app.json.loads  # Flask >= 2.2
    except Exception:
...
class Simulation(Resource):
    ...
    @api.expect(parser)
    @requires_auth("admin")
    def patch(self, sim_id: str, user: User | None = None):
        try:
...
class SimulationMeta(Resource):
    ...
    @requires_auth("admin")
    def delete(self, sim_id: str, user: User | None = None):
        try:
            data = request.get_json()

...
def sqlCreate():
    c.execute(
        "CREATE TABLE main.simulations (simulation_id INTEGER UNIQUE NOT NULL PRIMARY KEY ASC AUTOINCREMENT, "
    ...
    c.execute(
        "CREATE TABLE main.metadata (metadata_id INTEGER UNIQUE NOT NULL PRIMARY KEY ASC AUTOINCREMENT, "
    ...
    c.execute(
        "CREATE TABLE main.files (file_id INTEGER UNIQUE NOT NULL PRIMARY KEY ASC AUTOINCREMENT, "
    ...
    c.execute(
        "CREATE TABLE main.simulation_files (simulation_files_id INTEGER UNIQUE NOT NULL PRIMARY KEY ASC AUTOINCREMENT, "
...
def ingestFile(fileClass, fileName):
    ...
    if fileClass == "metadata":
        ...
        if not isValidUUID and isinstance(
            x[key], list
        ):  # Must be a maximum of 2 - a UUID and an Alias
            if len(x[key]) > 2:
    ...
    if not isValidUUID:
        if fileClass == "manifest":
            print(
                "ERROR ... The simulation uuid has not been specified in your simulation file manifest. Please use the UUID key for this identifier."
            )
        else:
            print(
                "ERROR ... The simulation uuid has not been specified in your simulation metadata file. Please use the IDENTIFIER key for this parameter."
            )
        return 1
    ...
    if sqlQuery(
        "select * from main.simulations where simulation_uuid = '" + uuid + "';'"
    ):
        print(
            "ERROR ... The simulation uuid provided is not registered in your IMAS database!"
        )
        return 1
